...
- Brief failure copy SHOULD be framed as a build-pipeline outcome, such as `Build failed: missing monster media`, instead of exposing raw audit internals.
- Media handoff copy SHOULD remain short, such as `Needs Module Media Generator`.
- Backend shaping SHOULD expose compact additive fields rather than passing through large nested report payloads.
