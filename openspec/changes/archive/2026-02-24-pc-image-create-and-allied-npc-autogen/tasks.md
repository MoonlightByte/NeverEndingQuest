...
- [ ] 12.1 Add allied NPC context hydration in `web/extensions/missing_media_autogen.py`:
...
- [ ] 12.2 Ensure generation callback passes hydrated context to `generate_and_save_portrait(...)`.

- [ ] 12.3 Add bounded archetype anchors in `core/toolkit/portrait_service.py`:
  - Include optional role/class-consistent visual cues when available.
  - Keep sanitation and prompt length bounds intact.

- [ ] 12.4 Preserve existing miss-path contracts:
  - Reuse-first behavior remains primary.
  - Allied-only policy, dedupe, cooldown, and non-blocking behavior remain unchanged.

- [ ] 12.5 Add regression coverage:
...
- Tasks 12.1 through 12.6: planned (NPC prompt enrichment after readiness alignment).
