...
### Requirement: Portrait prompt composition SHALL include bounded archetype anchors when role/class context is available

Portrait prompt composition SHALL include concise, sanitized archetype anchors derived from available role/class context to improve visual consistency for allied NPC generation.

#### Scenario: Class or role context available
- **WHEN** portrait prompt composition receives class or role context for a character
- **THEN** generated prompt includes role/class-consistent anchor cues
- **AND** anchor text remains bounded and sanitized

#### Scenario: Class or role context unavailable
- **WHEN** portrait prompt composition does not have class or role context
- **THEN** prompt composition continues without archetype anchors
- **AND** generation remains valid and non-failing

### SHOULD Guidance

- Keep archetype anchors concise and deterministic so they improve portrait quality without introducing prompt sprawl.
