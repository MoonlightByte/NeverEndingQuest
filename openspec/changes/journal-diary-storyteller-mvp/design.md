...
**Goals:**
- Add a persistent diary model with one active draft row and idempotent confirmed rows tied to saves or explicit Exit checkpoints.
...
`handle_start_game()`, `SaveGameManager.create_save_game()`, and explicit GUI exit handling SHALL call diary services behind guarded try/except blocks, log degraded outcomes, and continue success paths when diary generation fails.
...
- Failing explicit Exit on diary error: rejected because diary continuity is lower priority than reliable shutdown.
...
### Decision: Explicit Exit SHALL auto-confirm diary progress without creating a save snapshot
The explicit GUI Exit path SHALL confirm one diary checkpoint when new eligible source history exists after the last confirmed checkpoint, but SHALL NOT implicitly create a save-game artifact.

Rationale:
- Players often rely on Start Game / Exit as their normal session loop and may never click Save.
- Story So Far is confirmed-only, so confirmed diary progression must not depend entirely on manual save discipline.
- Auto-confirming the diary is much lighter-weight than auto-creating save files on every exit.

Alternatives considered:
- Require manual Save for all confirmed entries: rejected as poor UX because long-running campaigns can end up with zero confirmed entries.
- Make Exit perform a full or essential save automatically: rejected as too heavy and likely to create unwanted save clutter.
- Promote the draft to confirmed on the next Start Game: rejected because it delays canon confirmation and makes Exit semantics ambiguous.
...
- [Exit path regression or shutdown delay] -> Keep exit confirmation lightweight, idempotent, and fail-open; emit shutdown acknowledgment before any optional diary work.
- [Duplicate confirmed rows from repeated exit clicks] -> Use deterministic exit checkpoint identity and tests for idempotent reprocessing.
...
5. Append explicit Exit auto-confirm hook with idempotent checkpoint identity and fail-open behavior.
6. Add Journal API routes.
7. Add Journal Diary UI and download action.
8. Add tests for DB, service, route, and UI invariants.
...
- Whether explicit Exit auto-confirm should cover only the GUI Exit button or also future browser-close/session-abandonment heuristics. This plan assumes GUI Exit only.
