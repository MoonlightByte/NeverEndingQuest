...
def get_openai_client():
    """
    Create and return an OpenAI client configured based on model_config settings.

    Returns:
        OpenAI: Configured OpenAI client

    Behavior:
        - If USE_LM_STUDIO = True: Connects to localhost:1234 (local LM Studio)
        - If USE_LM_STUDIO = False: Connects to OpenAI API (requires config.OPENAI_API_KEY)
    ...
    """
    from model_config import USE_LM_STUDIO
...
def is_using_lm_studio():
    ...
    Returns:
        bool: True if USE_LM_STUDIO is enabled, False otherwise
