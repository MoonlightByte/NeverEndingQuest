...
def _resolve_targets(
    ...
    if all_modules:
        ...
        registered_existing = {
            slug for slug in registered if (modules_dir / slug).exists()
        }
...
def _run_schema_validation(
    module_slug: str, modules_dir: Path, schema_python: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "error": "Validation timed out after 120s"}
    except Exception as e:
...
def _run_publishability_audit(module_slug: str, modules_dir: Path) -> Dict[str, Any]:
    """Run layered publishability audit for a single module."""
    script_path = Path(__file__).parent / "audit_module_publishability.py"
    ...
    try:
        ...
        if result.stdout:
            try:
                ...
                return {
                    "success": False,
                    "exit_code": result.returncode,
                    "error": "Invalid JSON output from publishability audit",
                    "stdout": result.stdout[:500],
                    "stderr": result.stderr if result.stderr else None,
                }
        return {
            "success": False,
            "exit_code": result.returncode,
            "error": "No output from publishability audit",
            "stderr": result.stderr if result.stderr else None,
        }
        ...
        return {
            "success": False,
            "error": "Publishability audit timed out after 90s",
        }
    except Exception as e:
        return {
            "success": False,
...
def main():
    ...
    if args.module:
        targets = _resolve_targets(
            modules_dir, explicit_modules=args.module, json_mode=args.json
        )
    elif args.all:
    ...
    for slug in targets:
        ...
        # Run publishability audit (reporting only; readiness semantics preserved)
        publishability_result = _run_publishability_audit(slug, modules_dir)

        # Determine overall status
        schema_passed = (
            schema_result.get("success", False)
            and schema_result.get("exit_code", 1) == 0
        )
        audit_passed = (
            audit_result.get("success", False) and audit_result.get("exit_code", 1) == 0
        )
        ...
        continuity_passed = (
            continuity_result.get("success", False)
            and continuity_result.get("exit_code", 1) == 0
        )
        ...
        results[slug] = {
            ...
            "continuity": {
                ...
                "has_blocking_errors": bool(
                    (continuity_result.get("data") or {}).get("blocking_errors", [])
                ),
                "data": continuity_result.get("data"),
            },
            "publishability": {
                "passed": publishability_result.get("success", False)
                and (publishability_result.get("data") or {}).get("publishable_status")
                == "pass",
                "ready_status": (publishability_result.get("data") or {}).get(
                    "ready_status"
                ),
                "publishable_status": (publishability_result.get("data") or {}).get(
                    "publishable_status"
                ),
                "exit_code": publishability_result.get("exit_code"),
                "error": publishability_result.get("error"),
                "data": publishability_result.get("data"),
            },
        ...
        if not args.json:
            ...
            print(
                f"  {status} Audit: {'PASS' if audit_passed and not has_blocking_errors else 'FAIL'}"
            )
            print(f"  {status} Continuity: {'PASS' if continuity_passed else 'FAIL'}")
            publishable_state = (publishability_result.get("data") or {}).get(
                "publishable_status", "unknown"
            )
            ready_state = (publishability_result.get("data") or {}).get(
                "ready_status", "unknown"
            )
            print(
                f"  [INFO] Publishability: ready={ready_state} publishable={publishable_state}"
            )
            ...
            if publishability_result.get("error"):
                print(f"  Publishability Error: {publishability_result['error']}")

    # Aggregate continuity counts for summary
    continuity_passed = sum(
        1 for r in results.values() if r.get("continuity", {}).get("passed", False)
    )
    continuity_failed = sum(
        1 for r in results.values() if not r.get("continuity", {}).get("passed", False)
    )
    ...
    publishable_passed = sum(
        1
        for r in results.values()
        if r.get("publishability", {}).get("publishable_status") == "pass"
    )
    publishable_failed = sum(
        1
        for r in results.values()
        if r.get("publishability", {}).get("publishable_status") == "fail"
    )
    ...
    if args.json:
        summary = {
            ...
            "summary": {
                ...
                "publishability": {
                    "passed": publishable_passed,
                    "failed": publishable_failed,
                },
            },
        }
        ...
        print(f"\nPublishability:")
        print(f"  Publishable: {publishable_passed}")
        print(f"  Not publishable: {publishable_failed}")
