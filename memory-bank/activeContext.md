...
- **PC Leave/Return World Memory Planning (PLANNED - 2026-02-16):** OpenSpec change `pc-leave-return-world-memory` scaffolded with planning artifacts (`proposal.md`, `design.md`, `tasks.md`) and capability specs (`tt-pc-leave-return-lifecycle`, `memory-role-transition-continuity`). Goal is explicit retire/rejoin lifecycle with memory continuity persistence in `data/memory.db`, fail-open route integration in `web/routes/tabletop_party_routes.py`, and new transition service `core/memory/party_transition_memory.py`. Rollout is phase-gated with explicit verification checkpoints and merge-safe `# TABLETOP MODE:` host hooks.
