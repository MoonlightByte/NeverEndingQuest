...
### Owner-approved integration policy (2026-09-06)

The safety/main merge retains #284's transport recovery, changing progress,
structured deterministic-error handling, and usable turn loop after a terminal
provider error. The owner explicitly approved retaining the safety policy for
empty responses: the shared non-wizard provider path reissues the same logical
call without a six-empty terminal, until useful output or genuine supersession.
Empty output is not classified as an unhealable request error. This does not
remove separately ratified caller-specific completed-invalid policies, and it
