...
describe('MapTab', () => {
  ...
  it('disables the toolbar buttons until a map exists', () => {
    ...
    for (const b of container.querySelectorAll<HTMLButtonElement>('.neq-map-btn')) expect(b.disabled).toBe(true)
    ...
    rerender(<MapTab />)
    for (const b of container.querySelectorAll<HTMLButtonElement>('.neq-map-btn')) expect(b.disabled).toBe(false)
  })
  ...
  it('creates the map once per structural change, applies setRevealed before reveal(), and matches the createMap call shape', () => {
    ...
    expect(createMapMock).toHaveBeenCalledWith(
      ...
      payload().map,
      payload().area,
      expect.objectContaining({ uid: 'neqmap', fontCss: false, quiet: true }),
    )
  ...
  describe('React#7: marker only if currentLocationId is in revealed', () => {
    /** Appends a real [data-room] group (with a stubbed getBBox, since jsdom doesn't implement it) to `svg` so applyCurrentMarker has something to anchor on. */
    function appendRoomGroup(svg: SVGSVGElement, id: string): void {
      const g = document.createElementNS('http://www.w3.org/2000/svg', 'g')
      g.setAttribute('data-room', id)
      ;(g as unknown as { getBBox: () => DOMRect }).getBBox = () => ({ x: 0, y: 0, width: 20, height: 20 }) as DOMRect
      svg.append(g)
    }
